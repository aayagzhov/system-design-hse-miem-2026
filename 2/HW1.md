...
## 9. Итоги этапа «Сбор и анализ требований»

1. **Продукт** — read-heavy маркетплейс объявлений с тремя ролями и модерацией на входе.
2. **MVP** сфокусирован на core loop: публикация → модерация → поиск/просмотр → контакт.
3. **Масштаб** — 10M DAU, ~20K peak RPS (read), ~128 ТБ/день медиа.
4. **Критический NFR** — latency p95 ≤ 200ms на чтение, SLA 99,99%.
5. **Главный bottleneck** — read path (лента + поиск + CDN), не write.
6. **Ограничения 2026** — локализация данных, отечественное облако, регуляторика.

Данный документ будет использован как вход для HW2 (High Level Design, декомпозиция на сервисы, выбор БД).
